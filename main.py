...
class AnnonceModal(discord.ui.Modal, title="📢 Créer un Broadcast"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Créer un embed plus volumineux et professionnel
        embed = discord.Embed(
            color=color,
            timestamp=datetime.now()
        )
        ...
        # Message principal avec plus d'espace
        embed.description = f"\n{self.message.value}\n\n"
        ...
        # Thumbnail du serveur (si disponible)
        ...
        # Ajouter un champ vide pour plus d'espace
        embed.add_field(name="\u200b", value="\u200b", inline=False)
        
        # Envoyer dans le salon cible avec mention @everyone (optionnel)
...
async def on_ready():
    ...
    try:
        # Synchroniser globalement (visible pour tous)
    ...
    await bot.change_presence(
        activity=discord.Game(name="Revital RP | v4")
    )
...
async def verify(interaction: discord.Interaction, code: str):
    ...
    if captchas[uid]["code"] == code.upper().strip():
        # Code correct - retirer le rôle non-vérifié et ajouter le rôle vérifié
        del captchas[uid]
        save_data("data/captchas.json", captchas)
...
if __name__ == "__main__":
    create_data_files()
    bot.run("MTUxNzAwMjg3NzYzMjEyMjk4Mg.GXQ_QA.fH5T3h4P-sxh91npkNTWWnEyqUrj3I2B-iqnjA")